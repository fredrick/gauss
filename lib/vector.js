...
var Vector = function(values) {
  ...
  var cache = {
    ...
    'product': 1.0,
    'variance': 0.0,
    'values': Array.prototype.slice(vector)
  };

  cache.sync = function() {
    cache.values = Array.prototype.slice(vector);
  }
  ...
  Object.defineProperties(vector, {
    'sum': {
      value: function(callback) {
        var sum = 0.0;
        if (vector.equal(cache.values) && typeof cache.sum !== 'undefined' || cache.sum !== 0.0) {
          sum = cache.sum;
        } else {
    ...
    'product': {
      value: function(callback) {
        var product = 1.0;
        if (vector.equal(cache.values) && typeof cache.product !== 'undefined' || cache.product !== 1.0) {
    ...
    'variance': {
      value: function(callback) {
        var mean = 0.0,
            variance = 0.0;
        if (vector.equal(cache.values) && typeof cache.variance !== 'undefined' || cache.variance !== 0.0) {
    ...
    'push': {
      value: function() {
        var mean = cache.sum / vector.length;
            variance = cache.variance * vector.length;
